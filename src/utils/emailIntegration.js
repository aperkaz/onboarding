...
const sendInvitation = (from, recipient, subject, updateTransitionState, callback) => {
  let emailOpenTrack = `${URL}/public/transition/${recipient.tenantId}/${recipient.campaignId}/${recipient.id}?transition=read`;
  let data = {
	  to: recipient.email,
	  subject: subject,
	  html: `<!DOCTYPE html>
				<html>
					...
					<body style="border: 1px solid #ddd; font-family: avenir;">
						<a target="_blank" href="${URL}/public/landingpage/${recipient.tenantId}/${recipient.campaignId}/${recipient.id}?transition=loaded">
